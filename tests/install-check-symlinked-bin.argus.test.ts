...
function writeBaselineFixture(rootDir: string, externalExecutable: string) {
	...
	for (const agent of requiredAgents) {
		...
		if (agent === "zeus") {
			symlinkSync(externalExecutable, path.join(rootDir, "agents/bin", agent));
		} else {
...
describe("Argus: install-check rejects packaged launcher symlink escapes", () => {
	test("flags a Zeus launcher that is a symlink to an executable outside the package root", () => {
		...
		const externalExecutable = path.join(outsideDir, "payload.sh");
		writeExecutableScript(externalExecutable, "#!/bin/sh\necho pwned\n");

		writeBaselineFixture(rootDir, externalExecutable);

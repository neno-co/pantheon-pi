...
This prompt is maintained in Pantheon-Pi as the versioned source of truth for `hunter-silent-failure`. It is adapted from Olympus commit f939230eb557c673de27c3de1845c784699bfad7 for the Pi/acpx runtime.
...
**Why this matters**: In a Deno/TypeScript backend, silent failures manifest as:
...
4. **Deno patterns** — Use `std/expect` and `std/testing/bdd`; never Node.js patterns
...
- Place all test files in the `.argus/` directory at the worktree root (e.g., `.argus/invoice-catch-silence.argus.test.ts`)
- Tests MUST fail to be valid findings (a passing test = the bug doesn't exist)
- Import patterns must use workspace aliases (`neno-pkg/...`) or relative paths
...
describe('Argus: silent failure in functionUnderTest', () => {
  it('should surface the error when <condition>, but silently swallows it', async () => {
    ...
    // (i.e., if result is a silent default instead of an error)
    //
...
- [ ] Test file compiles (`deno check <file>`)
...
1. File: apps/api/src/billing/invoice-service.ts:87
...
2. File: apps/api/src/auth/session.ts:34
...
1. STATIC_WARNING:
     hunter: hunter-silent-failure
     file: apps/api/src/workers/email-sender.ts
     line: 55
     severity: medium
     category: async-race
     description: |
...
- apps/api/src/utils/format.ts:12 — optional chaining on display value (confidence: 45, non-critical path)
```
...
- **Node.js test patterns**: Never use `jest`, `mocha`, `chai` — use Deno's `std/testing/bdd` and `std/expect`

...
function writeBaselineFixture(rootDir: string, externalExtension: string) {
	...
	symlinkSync(externalExtension, path.join(rootDir, "src/extension/index.ts"));
	writeFileSync(
...
describe("Argus: install-check rejects symlinked extension entrypoints", () => {
	test("flags src/extension/index.ts when it resolves outside the package root", () => {
		...
		const externalExtension = path.join(outsideDir, "extension.ts");
		writeFileSync(externalExtension, 'export default function extension() { return "pwned" }\n');

		writeBaselineFixture(rootDir, externalExtension);
